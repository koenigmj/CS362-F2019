...
count++;
gainGoldOption = (rand () % 26);
discardOption = (rand () % 26); 		
//cardEffect(minion, choice1, choice2, choice3, &testG, handpos, &bonus);
playMinion(handPos, currentPlayer, gainGoldOption, discardOption, &testG);
	if (G.numActions > a)
...
if ((gainGoldOption >= 0)  && (G.coins == c + 2))
